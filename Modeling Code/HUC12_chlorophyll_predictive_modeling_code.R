...
#Merge chlorophyll-a data with HUC12 variable data, but keeping all HUC12 variable data
#Left join
...
library(openxlsx)
write.xlsx(HUC12_chlorophyll_a_corrected_join_full, 
           file = "chlor_a_predictor_variables_combined.xlsx")

##Extrapolate model to estimate high/low chlor-a across all of CONUS at HUC12 scale
#Filter for just HUC12s of unknown chlor-a corrected
...
HUC12_predict_corrected <- 
  na.omit(HUC12_predict_chlor_cor[,c(12,27,54,63)])
pred_randomForest2 <-  
...
HUC12s_predict_cor_chlora <- 
  cbind(HUC12_predict_corrected[,c(4)], pred_randomForest2)
